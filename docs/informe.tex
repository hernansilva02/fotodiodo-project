...
\section{Principio de Funcionamiento}
  ...
  % Dimensiones base
               \def\Hsd{1.6} % altura del semiconductor
               \def\W{8.0} % ancho total
               \def\xP{2.0} % fin de P
               \def\xI{5.0} % fin de I (inicio de N)

  % Contorno del semiconductor
               \draw[very thick] (0,0) rectangle (\W,\Hsd);

  % Rellenos (en escala de grises apta IEEE)
               \fill[gray!25]    (0,0) rectangle (\xP,\Hsd);          % P+
               \fill[gray!5]     (\xP,0) rectangle (\xI,\Hsd);         % I
               \fill[gray!35]    (\xI,0) rectangle (\W,\Hsd);          % N+

               \draw[dashed,gray!60] (\xP,0) -- (\xP,\Hsd);
               \draw[dashed,gray!60] (\xI,0) -- (\xI,\Hsd);

  % Contactos metálicos (superiores) y terminales
  % P+: metal
               \fill[black!60] (0.3,\Hsd) rectangle (1.7,\Hsd+0.3);
               \draw[very thick] (1.0,\Hsd+0.3) -- (1.0,\Hsd+1.0) node[above]{\small Ánodo (A)};
  % N+: metal
               \fill[black!60] (6.3,\Hsd) rectangle (7.7,\Hsd+0.3);
               \draw[very thick] (7.0,\Hsd+0.3) -- (7.0,\Hsd+1.0) node[above]{\small Cátodo (K)};

  % Etiquetas de regiones
           \node at ({1.0},{0.5*\Hsd}) {\large P$^{\boldsymbol{+}}$};
               \node at ({0.5*(\xP+\xI)},{0.5*\Hsd}) {\large I};
           \node at ({0.5*(\xI+\W)},{0.5*\Hsd}) {\large N$^{\boldsymbol{+}}$};

  % Ancho de la región intrínseca (Wi)
               \draw[|-|] (\xP,-0.35) -- (\xI,-0.35)
               node[midway,below=2pt] {$W_i$};
           \end{tikzpicture}
